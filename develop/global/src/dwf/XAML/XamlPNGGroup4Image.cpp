...
WT_Result WT_XAML_PNG_Group4_Image::serialize(
    WT_File &file) const
{
    ...
    if (rFile.serializingAsW2DContent() )
    {
        if ( !rFile.w2dContentFile() )
            ...
            return WT_PNG_Group4_Image::serialize( *rFile.w2dContentFile() );
    }

    if (rFile.heuristics().apply_transform())
    {
        const_cast<WT_XAML_PNG_Group4_Image*>(this)->transform(rFile.heuristics().transform());
    }
    ...
    pOut = NULL;
    WD_CHECK( prOpc->getPartOutputStream(
        m_format == WT_PNG_Group4_Image::PNG ? DWFMIME::kzMIMEType_PNG : DWFMIME::kzMIMEType_TIFF,
        oPath,
        &pOut) );
    if(pOut == NULL)
    {
    ...
    // for images - therefore let's use XamlImageRenditionPath.
    //
    ...
    WT_XAML_Rendition& rXamlRendition = static_cast<WT_XAML_Rendition &>(rFile.desired_rendition());

    WD_CHECK( rFile.dump_delayed_drawable() );
   
    WD_CHECK( rXamlRendition.sync(rFile, nParts) );    

    WD_CHECK( ((XamlDrawableAttributes::PathAttributeConsumer*)pPath)->consumeFill(&oFillProvider) );

    WD_CHECK( static_cast<WT_XAML_Visibility&>( rFile.rendition().visibility() ).providePathAttributes( ((XamlDrawableAttributes::PathAttributeConsumer*)pPath), rFile ) );
    WD_CHECK( static_cast<WT_XAML_URL&>( rFile.rendition().url() ).providePathAttributes( ((XamlDrawableAttributes::PathAttributeConsumer*)pPath), rFile ) );

    WD_CHECK( pPath->serialize(rFile) );

    //Now is the right time to serialize w2x stuff
    rFile.serializeRenditionSyncEndElement();
    pW2XSerializer->startElement(XamlXML::kpzPNG_Group4_Image_Element);
    ...
    // WT_Color_Map, b) feed it to the WT_XAML_PNG_Group4_Image ctor with
    // the copy parameter set to false.
    // if copy is true then WT_PNG_Group4_Image will create an internal
    // WT_Color_Map and drop the WT_XAML_PNG_Group4_Image one.
    //

    if ( format() == WT_PNG_Group4_Image::Group4X_Mapped )
    {
        if ( pCMap != NULL )
        {
...
WT_Result
WT_XAML_PNG_Group4_Image::parseAttributeList(XamlXML::tAttributeMap& rMap, WT_XAML_File& rFile)
{
	if(!rMap.size())
		return WT_Result::Internal_Error;

	//<PNG_Group4_Image refName="N43" Format="12" Ref="c:\Poseidon\dwfx\W2dTest\out\616E467F-EF70-47FA-AE9C-E66B5172B132.png"
    //    Width="252" Height="232" Area="2600,318,2852,550" />
