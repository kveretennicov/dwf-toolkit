...
WT_Result WT_XAML_Text_HAlign::serialize(WT_File & file) const
{
    ...
    if (rFile.serializingAsW2DContent() )
    {
        if ( !rFile.w2dContentFile() )
            ...
            return WT_Text_HAlign::serialize( *rFile.w2dContentFile() );
    }
    ...
    pW2XSerializer->startElement(XamlXML::kpzText_HAlign_Element );

    DWFString zStr( enum_to_string( hAlign() ) );
...
WT_Result
WT_XAML_Text_HAlign::parseAttributeList(XamlXML::tAttributeMap& rMap, WT_XAML_File& /*rFile*/)
{
	if(!rMap.size())
		return WT_Result::Internal_Error;
