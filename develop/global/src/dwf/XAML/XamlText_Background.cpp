...
WT_Result WT_XAML_Text_Background::serialize(WT_File & file) const
{
    ...
    if (rFile.serializingAsW2DContent() )
    {
        if ( !rFile.w2dContentFile() )
            ...
            return WT_Text_Background::serialize( *rFile.w2dContentFile() );
    }
    ...
    pW2XSerializer->startElement(XamlXML::kpzText_Background_Element );

    DWFString zStr( enum_to_string( background() ) );

    pW2XSerializer->addAttribute(XamlXML::kpzValue_Attribute, zStr );

    if (offset() > 0)
    {
        pW2XSerializer->addAttribute(XamlXML::kpzOffset_Attribute,(int) offset() );
    }
...
WT_Result
WT_XAML_Text_Background::parseAttributeList(XamlXML::tAttributeMap& rMap, WT_XAML_File& /*rFile*/)
{
	if(!rMap.size())
		return WT_Result::Internal_Error;
    ...
    {
        string_to_enum( *ppValue, background() );
    }
    else
    {
    ...
    ppValue = rMap.find( XamlXML::kpzOffset_Attribute );
    if ( ppValue!=NULL && *ppValue!=NULL )
    {
        offset() = (WT_Integer32) atoi( *ppValue );
    }
