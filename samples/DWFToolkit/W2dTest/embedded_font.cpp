...
//
// $Header: /Components/Internal/DWF Toolkit/v7/samples/W2dTest/embedded_font.cpp 2     1/04/05 1:45p Klugj $
...
WT_Result write_embedded_font(WT_File & whip_file)
{
    ...
    //Embed a font
    // The toolkit has updated, the embed font will not add to the w2dx. It just need to add the font resource and register the name and file path.
    // So the follow code is invalid, we should add the embed font as in function write_test (XPSTest.cpp).
